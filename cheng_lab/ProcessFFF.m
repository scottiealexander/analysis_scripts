function data = ProcessFFF(base_dir,varargin)

% ProcessTune
%
% Description:
%
% Syntax: ProcessFFF(base_dir)
%
% In:
%
% Out:
...
for kD = 1:ndir
    run_dir = FindDirs(cell_dir{kD},['.*_chap_\d+']);
    ...
    for kC = 1:numel(run_dir)
        ...
        if ~exist(ofile,'file') || opt.force            
            ...
            ifo = [];
            for kU = 1:numel(cts)
                tmp = FFFlash(GetSMRFile(run_dir{kC}),'ts',cts{kU});
                if isempty(ifo)
                    ifo = tmp;
                else
                    ifo(kU,1) = tmp;    
...
function f = GetSMRFile(x)
    ...
    if ~exist(f,'file')
        ...
        str1 = sprintf('%s%02dP%02d_%d_chap',grp,s.k,s.age,s.ncell);
